  ...
  return (
    <div className="space-y-6 p-4 rounded-xl" style={{ background: 'var(--mizan-surface)', border: '1px solid var(--mizan-border)' }}>
      {/* Enable Prayer Reminders */}
       <div dir="ltr" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '12px 0', borderBottom: '1px solid var(--mizan-border)', gap: '12px' }}>
         ...
         <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
           <div className={`flex items-center gap-3 ${isRTL ? 'justify-end' : 'justify-start'}`}>
             ...
             <div>
               <p className="font-medium text-sm" style={{ color: 'var(--mizan-text)' }}>{language === 'ar' ? 'تنبيهات الصلاة' : 'Prayer Reminders'}</p>
               <p className="text-xs mt-1" style={{ color: 'var(--mizan-text-secondary)' }}>{language === 'ar' ? 'احصل على تذكيرات قبل دخول وقت الصلاة' : 'Get reminded before prayer time'}</p>
             </div>
           </div>
         </div>
       </div>
          ...
          <div dir="ltr" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '12px 0', borderBottom: '1px solid var(--mizan-border)', gap: '12px' }}>
            ...
            <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
              <div className={`flex items-center gap-3 ${isRTL ? 'justify-end' : 'justify-start'}`}>
                ...
                <div>
                  <p className="text-sm font-medium" style={{ color: 'var(--mizan-text)' }}>{language === 'ar' ? 'ساعات الصمت' : 'Silent Hours'}</p>
                  <p className="text-xs mt-1" style={{ color: 'var(--mizan-text-secondary)' }}>{language === 'ar' ? 'عدم إرسال التنبيهات في أوقات معينة' : 'No reminders during these times'}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div dir="ltr" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '12px 0', borderTop: '1px solid var(--mizan-border)', marginTop: '12px', gap: '12px' }}>
        <Switch checked={settings.notify_prayer !== false} onCheckedChange={(value) => handleToggle('notify_prayer', value)} disabled={isSaving} />
        <div style={{ flex: 1, textAlign: isRTL ? 'right' : 'left' }}>
          <div className={`flex items-center gap-3 ${isRTL ? 'justify-end' : 'justify-start'}`}>
            <Bell className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--mizan-emerald)' }} />
            <p className="font-medium" style={{ color: 'var(--mizan-text)' }}>{language === 'ar' ? 'إشعارات الصلاة العامة' : 'General Prayer Notifications'}</p>
          </div>
        </div>
      </div>
    </div>
